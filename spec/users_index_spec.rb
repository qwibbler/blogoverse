...
RSpec.describe 'Users Page', type: :system do
  before :each do
    User.create(name: 'user1', email: 'user1@example.com', password: 'password',
                photo: 'https://images.unsplash.com/photo-1616587894289-86480e533129', confirmed_at: Time.now)
    User.create(name: 'user2', email: 'user2@example.com', password: 'password',
                photo: 'https://images.unsplash.com/photo-1503945438517-f65904a52ce6', confirmed_at: Time.now)
    Post.create(user: User.first, title: 'title-first', text: 'test')
    Post.first.update_posts_count
  end
  ...
  describe 'has index of all users' do
    ...
    it 'cans see the profile picture for each user' do
